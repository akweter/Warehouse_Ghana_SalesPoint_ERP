...
// Update quotation row when already exist in DB row
const updateExistingQuote = async (payload, invoiceNumber) => {
	const sql = `
		UPDATE
			invoice
		SET ? 
		...
		AND 
			Inv_status = "PROFORMA INVOICE"
	`;
	return await executeQuery(sql, [payload, invoiceNumber]);
}
