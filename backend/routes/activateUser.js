...
const activateUser = async(email, userID) =>{
    try {
        const sql = "UPDATE usermanagement SET activated = 'yes' WHERE Usr_email = ?";
        return await executeQuery(sql, email);
    } catch (err) {
        await logErrorMessages(`Unable to activate user: ${email}, error: ${JSON.stringify(err)}`, userID);
        return null;
    }
}
  ...
  const getUserByEmail = async(email, userID) =>{
    try {
        ...
        await logErrorMessages(`Couldn't query for user: ${email}, error: ${JSON.stringify(error)}`, userID);
        return null;
    }
}
...
Auth.get("/", async (req, res) => {
    ...
    if (verifyToken(token)) {
            ...
            if (user.length > 0) {
                user.map(e=>{
                    if (e.activated === 'no') {
                        ...
                        activateUser(email, sanitizedData.accountId)
                        .then(()=>{
                            logMessage(`Email: ${email} has activated successfully`, e.Usr_id);
                        ...
                        .catch((err)=>{
                            logErrorMessages(`error while activating ${email} with the error ${JSON.stringify(err)}`, e.Usr_id);
                            return res.json({status: 'error', message: 'Oops! Something went wrong. Log in again'});
                        });
                    }
                    else{
                        logErrorMessages(`Email: ${email} attempted to activate the account, but it's already active.`, e.Usr_id);
                        return res.json({status: 'error', message: 'You are already activated. Kindly log in'});
                    }
                });
            }
            else{
                await logErrorMessages(`Email: ${email} not found from the query result decoding from token`, email);
                return res.json({status: 'error', message: 'Something went bad. Please sign up again!'});
            }
        }
        else {
            await logErrorMessages(`No email found in the token: ${decrypted}`, email);
        ...
        await logErrorMessages(`Verifying token error ${token}`, 'NA');
